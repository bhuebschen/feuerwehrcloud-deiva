...
namespace WhatsAppApi
{
    ...
    public class WhatsApp : WhatsSendBase
    {
		...
		public void SendMessageLocation (string jid, double lat, double lng, string name = "", string url = "")
		{
			FMessage fMessage = new FMessage (WhatsApp.GetJID (jid), true) {
				data = name
			};
			fMessage.latitude = lat;
			fMessage.longitude = lng;
			fMessage.location_details = name;
			fMessage.media_wa_type = FMessage.Type.Location;
			this.SendMessageWithMedia (fMessage);
		}
